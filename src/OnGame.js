...
const OnGame = ({
    ...
    const [isPlayerAlreadyAttacked, setIsPlayerAlreadyAttacked] =
        useState(false);
    ...
    const randomMovement =
        "transform 800ms cubic-bezier( 0.79, 0.33, 0.14, 0.53 )";
    const countdownElement = document.getElementById("countdown");
    const typewriterContainer = document.getElementsByClassName(
        "typewriter_container"
    )[0];
    const payOffMessageElement =
        document.getElementsByClassName("payoff_message")[0];

    const player1PortraitImage =
        document.getElementsByClassName("player_1_portrait")[0];
    const player2PortraitImage =
        document.getElementsByClassName("player_2_portrait")[0];
    ...
    const setupWebcam = () => {
        navigator.mediaDevices
            ...
            .then(function (stream) {
                ...
                document.getElementById("webcam").onloadedmetadata = function (
                    e
                ) {
    ...
    useEffect(() => {
        ...
        if (isPlayerAlreadyAttacked) {
            return;
        }
        if (yoloDetected.value === "") {
            return;
        }
    ...
    useEffect(() => {
        ...
        setPlayerAttackHistory((prevPlayerAttackHistory) => {
            const updatedHistory = [
                playerAttack,
                ...prevPlayerAttackHistory.slice(0, maximumHistoryLength - 1),
            ];
    ...
    useEffect(() => {
        ...
        let randomAiAttack;
        if (difficulty === "hard") {
            let AiAttackProbabilities =
                AIPlayer.adaptAiAttack(playerAttackHistory);
            randomAiAttack =
                choices[AIPlayer.weightedRandom(AiAttackProbabilities)];
        } else {
    ...
    useEffect(() => {
        ...
        if (payoff === 1) {
            ...
            payOffMessageElement.innerHTML = `${playerAttack} beats ${
                choices[aiAttack - 1]
            }. You Win!`;
            ...
            payOffMessageElement.innerHTML = `${playerAttack} draws with ${
                choices[aiAttack - 1]
            }. It's a draw!`;
    ...
    useEffect(() => {
        const countdownContainer = document.getElementsByClassName(
            "countdown_container"
        )[0];
    ...
    const changeConfThresh = (newConfThresh) => {
        if (newConfThresh === confThreshSelected) {
            return;
        }
        ...
        >
            ...
            <div className="nameplate_container">
                <div className="player_1_nameplate_container">
                    <div className="player_1_username">
                        <StringEffect text="Player" />
                    </div>
                    <img
                        className="player_1_nameplate"
                        src={player1Nameplate}
                        alt=""
                    />
                    <img
                ...
                <div className="score_container">
                    <div ref={player1ScoreRef} className="score">
                        <StringEffect text={scores[0]} />
                    </div>
                    <div className="separator">
                        <StringEffect text=":" />
                    </div>
                    <div ref={player2ScoreRef} className="score">
                        <StringEffect text={scores[1]} />
                    </div>
                </div>
                <div className="player_2_nameplate_container">
                    <div className="player_2_username">
                        <StringEffect text="A.I." />
                    </div>
                    <img
                        className="player_2_nameplate"
                        src={player2Nameplate}
                        alt=""
                    />
                    <img
                        className="player_2_portrait"
                        src={portraitAi}
                        alt=""
                    />
                </div>
            ...
            <div className="payoff_message_container">
                <img
                    className="payoff_message_background"
                    src={textbox}
                    alt=""
                />
